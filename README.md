...
支持**本地 txt / epub 阅读**和**在线搜索阅读七猫、书旗小说**，带阅读历史（记住读到第几章 + 滚动进度）、
字体调节、夜间模式。
...
在线部分内置**七猫**和**书旗**：搜索框输入书名即可跨本地 + 七猫 + 书旗聚合搜索。
...
### 🔍 搜索
在搜索框输入**书名关键词**，跨本地书库、七猫和书旗在线聚合搜索。
...
七猫的付费章节未解锁时，适配器 `raise ChapterLockedError`。书旗源使用移动端
`platform=0` 的目录与正文接口，并按移动端返回的免费状态读取章节。
